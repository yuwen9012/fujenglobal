<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($id) && isset($newOrder)) {
            // 獲取原來的 num_order
            $query = "SELECT `num_order` FROM `$dataSheet` WHERE id = ?";
            ...
            if ($currentRow) {
                ...
                if ($currentOrder != $newOrder) {
                    // 調整其他行的 num_order
                    if ($currentOrder < $newOrder) {
                        $updateQuery = "UPDATE `$dataSheet` SET `num_order` = `num_order` - 1, `update_time` = NOW() WHERE `num_order` > ? AND `num_order` <= ?";
                    ...
                    // 更新當前行的 num_order
                    $query = "UPDATE `$dataSheet` SET `num_order` = ?, `update_time` = NOW() WHERE `id` = ?";
                    $stmt = $mysqli->prepare($query);
